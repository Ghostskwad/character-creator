class Rogue < ApplicationRecord
    ...
    MAX_STAT_VALUE = 20
    MIN_STAT_VALUE = 1
    MIN_DEXTERITY_VALUE = 10
    MIN_CHARISMA_VALUE = 5
    
    # Callback to randomize stats before a new Rogue is created
    ...
    def randomize_stats
      loop do # Ensures that all stats are greater than 0
        ...
        # Prioritize dexterity and charisma to ensure minimum values are met
        stats[:dexterity] = rand(1..[remaining_points, MAX_STAT_VALUE - 5].min) 
        stats[:dexterity] = [stats[:dexterity], MIN_DEXTERITY_VALUE].max
        remaining_points -= stats[:dexterity]
      
        stats[:charisma] = rand(1..[remaining_points, MAX_STAT_VALUE - stats.values.sum].min)
        ...
        stats.keys.reject { |stat| [:dexterity, :charisma].include?(stat) }.each do |stat|
        ...
        if stats.values.all? { |value| value > 0 }
          # Set the randomized stats as attributes for the new Rogue
