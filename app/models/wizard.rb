class Wizard < ApplicationRecord
    ...
    MAX_STAT_VALUE = 20
    MIN_STAT_VALUE = 1
    MIN_INTELLIGENCE_VALUE = 10
    MIN_WISDOM_VALUE = 5
    
    # Callback to randomize stats before a new Wizard is created
    ...
    def randomize_stats
      loop do # Ensures that all stats are greater than 0
        ...
        # Prioritize intelligence and wisdom to ensure minimum values are met
        stats[:intelligence] = rand(1..[remaining_points, MAX_STAT_VALUE - 5].min) 
        stats[:intelligence] = [stats[:intelligence], MIN_INTELLIGENCE_VALUE].max
        remaining_points -= stats[:intelligence]
      
        stats[:wisdom] = rand(1..[remaining_points, MAX_STAT_VALUE - stats.values.sum].min)
        ...
        stats.keys.reject { |stat| [:intelligence, :wisdom].include?(stat) }.each do |stat|
        ...
        if stats.values.all? { |value| value > 0 }
          # Set the randomized stats as attributes for the new Wizard
