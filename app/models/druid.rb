class Druid < ApplicationRecord
    ...
    # Callback to randomize stats before a new Druid is created
    ...
    def randomize_stats
      loop do # Ensures that all stats are greater than 0
        ...
        # Prioritize wisdom and constitution to ensure minimum values are met
        ...
        stats.keys.reject { |stat| [:wisdom, :constitution].include?(stat) }.each do |stat|
        ...
        if stats.values.all? { |value| value > 0 }
          # Set the randomized stats as attributes for the new Druid
